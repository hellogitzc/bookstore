	...
	$(function() {
		...
		function checkStreet() {
			...
			if (n.test(street.val()) || o.test(street.val())
					|| p.test(street.val())) {
				street.focus(), street.siblings(".tipMsg").html('详细地址不正确')
						.show();
				return false;
			}
		...
		$('#J_editAddrOk')
				.click(
						function() {
							var Province = $(".select-1").find(
									"option:selected").text();
							var City = $(".select-2").find("option:selected")
									.text();
							var County = $(".select-3").find("option:selected")
									.text();
							var c = $("#Consignee"), g = $("#Street"), h = $("#Zipcode"), t = $("#Telephone"), j = $("#Tag"), i = $("#ID");
		...
		$(".J_editAddr").click(
				function() {
					...
					$consignee = $p.find('.itemConsignee').html(), $tel = $p
							.find('.itemTel').html(), $tag = $p
							.find('.itemTag').html(), $street = $p.find(
							'.itemStreet').html(), $zipcode = $p.find(
							'.zipcode').html(), $province = $p
							.find('.province').html(), $city = $p.find('.city')
							.html(), $county = $p.find('.county').html();
					...
					$('#Consignee').val($consignee), $("#Telephone").val($tel),
							$("#Street").val($street), $("#Tag").val($tag), $(
									"#Zipcode").val($zipcode), $("#ID")
									.val($id),
		...
		$("#checkoutPaymentList").find('.item').click(
				function() {
					var b = $(this).children('input[name="pay"]');
					$(this).parents("#checkoutPaymentList").find('.item')
							.removeClass("selected");
					$(this).addClass("selected");

				});
		$("#checkoutShipmentList").find('.item').click(function() {
			var b = $(this).children('input[name="shipment"]');
		});
		// 订单结算时
		$("#checkoutToPay").click(function() {
              var isAdd = 0;
              
			$("#checkoutPaymentList").find('.item').each(function() {
				if ($(this).hasClass('selected')) {
					
					$('#status').val($(this).find('.choice').val());
					
				}
				
			});
             
			$("#checkoutAddrList").find('.item').each(function() {
				if ($(this).hasClass('selected')) {
					// alert($(this).find('.userid').val());
					var $addressid = $(this).find('.addressId').val();
					$('#addressid').val($addressid);
					
					var $addressdetial =$(this).find('.addressdetial').val();
					
					$('#addressdetial').val($addressdetial);
					isAdd=1;
				}
			});
			...
			$("#checkoutGoodsList").find('.item').each(function() {
				...
				// alert($(this).find('.bookid').val());

			})
			$('#t_price').val($('#totalPrice').html());
